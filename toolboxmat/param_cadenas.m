...
%See also GIRO_BASE, CILINDRO_BASE
function [matriz_x, matriz_y, matriz_z ] = param_cadenas( indices_braid,n )
    ...
    if(max(abs(indices_braid)) < n)
        %resto de cadenas "triviales"
        for cadena_actual=max(abs(indices_braid))+1:1:n-1
              x = x_cil+2*cadena_actual; y = y_cil; z = z_cil;
              for i=2:1:length(indices_braid)
                    siguiente_x = x_cil+2*cadena_actual;%falta algo contador
                    siguiente_y = y_cil;
                    siguiente_z = z_cil-3*pi*(i-1);
                    x = [siguiente_x,x];
                    y = [siguiente_y,y];
                    z = [siguiente_z,z];
              end
