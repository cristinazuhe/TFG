N_cortes = 20;
Radio = 0.5;
indices_braid1=[ +2 -3 +3 -1 ];
indices_braid2=[+2 0 0 -1 ];
indices_braid3=[-1];
eliminar = false;
 ...
 %Obtenemos las parametrizaciones de las cuerdas. 
    ...
    %Ahora voy a borrar las trenzas sobrantes a derechas.
    hold off;   
      for t=0:0.1:1 
        for j=1:1:size(matriz_x2,1)
            if(j==size(matriz_x2,1))  %ahora mismo lo voy a poner para que siempre se borre la última cadena       
