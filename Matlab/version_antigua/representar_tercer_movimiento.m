function [p0,p1,p2,p3 ] = representar_tercer_movimiento(indices_braid, N_cortes, Radio, crossing )
    ...
    for i=1:length(indices_braid)
        ...
        %antes de llegar al crossing cruze, puede que tengamos cilindros
        for j=1:abs(cruce)-1
            [p0,plt0] = cilindro_braid(j+contador,inicio,fin,N_cortes,Radio); %Representamos los cilindros
            hold on;
            contador = contador +1;
            if (abs(indices_braid(crossing)) < abs(indices_braid(crossing+1)))%Si el primer cruce visual es más alto(menor valor)...
                if i == (crossing+1)     %Si estoy en el segundo cruce, tendré que borrar los cilindros bajo el primer cruce
                    indice = abs(indices_braid(crossing)); %obtengo el numero de trenza primero del primer cruce
                    if j == (indice + 1) || j==indice      %borro los cilindros correspondientes de este indice y el siguiente.
                        delete ([p0,plt0])
                    end
                end
            end
            if (abs(indices_braid(crossing)) > abs(indices_braid(crossing+1))) %Si el primer cruce visual es más bajo...
                if i == (crossing)      %Si estoy en el segundo cruce (primer crossing), tendré que borrar los cilindros superiores al primer cruce 
                    indice = abs(indices_braid(crossing+1)); %obtengo el numero de trenza primero del primer cruce (segundo crossing)
                    if j == (indice + 1) || j==indice   %borro los cilindros correspondientes de este indice y el siguiente. 
                        delete ([p0,plt0])
                    end
                end
        ...
        if(cruce < 0) %Si el cruce es negativo, represento trenza cruzada negativa
                [p1,plt1,x1,y1,z1,x1_cil,y1_cil,z1_cil,...
                 ...
                 hold on;
                 %obtengo las funciones de cilindro y de cruce del crossing 
                 if(i== crossing) 
                 ...
                 %obtengo las funciones de cilindro y de cruce del crossing+1
                 if(i== (crossing+1))  
                   delete ([p1,plt1,p2,plt2]);
        ...
        else   %Si el cruce es negativo, represento trenza cruzada positiva
                 [p1,plt1,x1,y1,z1,x1_cil,y1_cil,z1_cil,...
        ...
        %despues del crossing cruze, puede que tengamos cilindros.
        %similar a los cilindros que representamos anteriores al cruce. 
  ...
  %Cada trozo de trenza de crossing y crossing+1 (un total de 8 trozos)
  %tendrá una función cilindro y una funcion cruce (cross). Tendremos que
  %ir de una función a otra según nos vaya interesando en cada trozo de
  %trenza. 
